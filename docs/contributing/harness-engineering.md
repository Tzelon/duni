...
- Humans steer outcomes; agent provide guidance unless request then agents execute implementation details.
...
For this repo, the default evaluation step is `zig build`.
...
1. **Build** — when the violation is local and syntactic (or
...
- Deterministic scripts (targeted tests, type checks).
