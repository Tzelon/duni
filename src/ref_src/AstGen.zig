    ...
    switch (tree.nodeTag(member_node)) {
        ...
        => {
            ...
            const body: Ast.Node.OptionalIndex = if (tree.nodeTag(member_node) == .fn_decl)
                ...
                .none;
            try fnDecl(astgen, gz, scope, member_node, body, full);
        },
        else => unreachable,
    }
}
...
fn addExtraAssumeCapacity(astgen: *AstGen, extra: anytype) u32 {
    const result: u32 = @intCast(astgen.extra.items.len);
    inline for (std.meta.fields(@TypeOf(extra))) |field| {
        astgen.extra.appendAssumeCapacity(switch (field.type) {
            u32 => @field(extra, field.name),
            Dir.Inst.Ref,
            Dir.Inst.Index,
            Dir.NullTerminatedString,
            => @intFromEnum(@field(extra, field.name)),
            i32, Ast.Node.Offset, Ast.TokenOffset => @bitCast(@field(extra, field.name)),
...
/// Fills the `extra` payload for a `declaration` instruction and unstacks `value_gz`.
fn setDeclaration(
    decl_inst: Dir.Inst.Index,
    args: struct {
        name: Dir.NullTerminatedString,
        /// Must have nothing stacked on top of it.
        value_gz: *GenZir,
    },
) !void {
    ...
    const value_len: u32 = @intCast(value_body.len);

    const payload_index = try astgen.addExtra(Dir.Inst.Declaration{
        .name = args.name,
        .body_len = value_len,
    });
    try astgen.extra.ensureUnusedCapacity(gpa, value_body.len);
    for (value_body) |inst| astgen.extra.appendAssumeCapacity(@intFromEnum(inst));

    astgen.instructions.items(.data)[@intFromEnum(decl_inst)].declaration.payload_index = payload_index;
