...
pub const Wip = struct {
    ...
    pub fn addDirErrorMessages(
        ...
        const payload_index = dir.extra[@intFromEnum(Dir.ExtraIndex.compile_errors)];
        assert(payload_index != 0);

        const header = dir.extraData(Dir.Inst.CompileErrors, payload_index);
        const items_len = header.data.items_len;
        var extra_index = header.end;
        for (0..items_len) |_| {
            const item = dir.extraData(Dir.Inst.CompileErrors.Item, extra_index);
            ...
            const err_span = blk: {
                if (item.data.node.unwrap()) |node| {
                    ...
                    break :blk Ast.Span{ .start = start, .end = end, .main = start };
                } else unreachable;
            };
            const err_loc = std.zig.findLineColumn(source, err_span.main);

            {
                const msg = dir.nullTerminatedString(item.data.msg);
                try eb.addRootErrorMessage(.{
                    ...
                    .notes_len = item.data.notesLen(dir),
                });
            }

            if (item.data.notes != 0) {
                const notes_start = try eb.reserveNotes(item.data.notesLen(dir));
                const block = dir.extraData(Dir.Inst.Block, item.data.notes);
                const body = dir.extra[block.end..][0..block.data.body_len];
                for (notes_start.., body) |note_i, body_elem| {
                    const note_item = dir.extraData(Dir.Inst.CompileErrors.Item, body_elem);
                    const msg = dir.nullTerminatedString(note_item.data.msg);
                    const span = blk: {
    ...
    fn setExtra(wip: *Wip, index: usize, extra: anytype) void {
        ...
        var i = index;
        inline for (extra_info.fields) |field| {
